...
def tomar_comando():
    try:
        with sr.Microphone() as fonte: #aqui estamos inserindo o input <entrada> do microfone como fonte do input, no caso a minha voz
            print('Ouvindo...')
            audio = ouvinte.listen(fonte) #criando a fonte audio, ou seja, ele vai ouvir o que foi falado e armazenado como fonte
            comando = ouvinte.recognize_google(audio, language='pt-BR') #o comando vai reproduzir o audio, configurado em PTBR
            if 'roda' in comando:
